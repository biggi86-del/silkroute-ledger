  ...
  const rows = useMemo(() => {
    ...
    return items
      .map((item) => {
        ...
        // Use real sell price if available, otherwise estimate from dest buy price
        const effectiveSellPrice = actualSellPrice ?? destBuyPrice;
        const profit = buyPrice !== null && effectiveSellPrice !== null
          ? effectiveSellPrice - buyPrice : null;

        const profitConfirmed = actualSellPrice !== null;
        const isLocalSell = (actualSellEntry ?? destBuyEntry)?.local === true;

        return {
          item, buyPrice, destBuyPrice, actualSellPrice,
          effectiveSellPrice, profit, profitConfirmed,
          buyEntry, destBuyEntry, actualSellEntry, isLocalSell,
        };
      })
      ...
      .sort((a, b) => {
        ...
        // Confirmed sell data ranks above estimated
  ...
  return (
    ...
    >
      ...
      {bestTrade && buyCity !== sellCity && (
        <div style={{
          background: "rgba(74,124,89,0.1)", border: "1px solid rgba(74,124,89,0.35)",
          ...
          <div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "0.8rem", color: "var(--text-dim)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.25rem" }}>
              ...
              {bestTrade.profitConfirmed && (
                <span style={{ marginLeft: "0.5rem", background: "rgba(74,124,89,0.2)", border: "1px solid rgba(74,124,89,0.4)", color: "var(--profit-light)", padding: "0.1rem 0.4rem", borderRadius: 2, fontSize: "0.65rem" }}>
                  ✓ CONFIRMED
                </span>
              )}
            </div>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "1.3rem", color: "var(--parchment)" }}>
              {bestTrade.item}
            </div>
          </div>
          <div style={{ display: "flex", gap: "2rem" }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div className="price-num" style={{ fontSize: "1.1rem", color: "var(--parchment)" }}>
            ...
            {bestTrade.actualSellPrice !== null ? (
              <div style={{ textAlign: "center" }}>
                ...
                <div className="price-num" style={{ fontSize: "1.1rem", color: "var(--gold)" }}>
                  {bestTrade.actualSellPrice?.toLocaleString()}
                </div>
              </div>
            ) : (
              <div style={{ textAlign: "center" }}>
                ...
                <div className="price-num" style={{ fontSize: "1.1rem", color: "var(--parchment)" }}>
                  {bestTrade.destBuyPrice?.toLocaleString()}
                </div>
              </div>
            )}
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "0.7rem", color: "var(--text-dim)" }}>PROFIT / UNIT</div>
              <div className="price-num" style={{ fontSize: "1.4rem", color: "var(--profit-light)", fontWeight: 600 }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="ledger-table">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={7} style={{ textAlign: "center", color: "var(--text-dim)", padding: "2rem", fontFamily: "'Cormorant Garamond', serif" }}>
                    {buyCity === sellCity ? "Select two different cities to compare." : "No price data available for this route."}
                  </td>
                </tr>
              ) : (
                rows.map((row, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="price-num">
                        {row.buyPrice !== null ? row.buyPrice.toLocaleString() : <span style={{ color: "var(--text-dim)" }}>—</span>}
                      </td>
                      {/* Buy price in destination */}
                      <td className="price-num" style={{ color: "var(--text-dim)" }}>
                        {row.destBuyPrice !== null ? row.destBuyPrice.toLocaleString() : <span>—</span>}
                      </td>
                      {/* Actual sell price in destination */}
                      <td className="price-num" style={{ color: row.actualSellPrice !== null ? "var(--gold)" : "var(--text-dim)" }}>
                        {row.actualSellPrice !== null
                          ? <span title="Confirmed sell price from scan data">{row.actualSellPrice.toLocaleString()} <span style={{ fontSize: "0.65rem", color: "var(--profit-light)" }}>✓</span></span>
                          : <span style={{ fontSize: "0.7rem" }}>—</span>}
                      </td>
                      {/* Profit */}
                      <td className="price-num" style={{ color: hasProfit ? "var(--profit-light)" : hasLoss ? "var(--loss-light)" : "var(--text-dim)", fontWeight: hasProfit ? 600 : undefined }}>
                        {row.profit !== null ? `${row.profit > 0 ? "+" : ""}${row.profit.toLocaleString()}` : "—"}
                        {row.profit !== null && !row.profitConfirmed && (
                      ...
                      <td>
                        {row.profit !== null && row.profit !== 0 ? (
                          <span style={{ fontSize: "0.72rem", color: hasProfit ? "var(--profit-light)" : "var(--loss-light)" }}>
                            {hasProfit ? `${buyCity} → ${sellCity}` : `${sellCity} → ${buyCity}`}
                          </span>
                      ...
                      <td>
                        {row.isLocalSell && (
                          <span title="Locally produced — sells for less here" style={{ cursor: "help", fontSize: "0.85rem", display: "inline-flex", alignItems: "center", gap: "0.3rem", color: "var(--gold)" }}>
      ...
      >
        ✦ Profit uses actual Sell price when scan data exists (✓), otherwise estimated from Buy price difference. "est." = estimated.
      </div>
    </PageWrapper>
  );
}
